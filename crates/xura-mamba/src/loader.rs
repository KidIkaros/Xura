...
impl std::fmt::Display for LoadError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            LoadError::ShapeMismatch { key, expected, got } => write!(
                f,
...
fn get_tensor(tensors: &SafeTensors<'_>, key: &str) -> Result<Vec<f32>, LoadError> {
    let view = tensors
        .tensor(key)
        .map_err(|_| LoadError::MissingKey(key.to_string()))?;
...
pub fn load_pretrained(model_dir: &Path) -> Result<MambaLMHeadModel, LoadError> {
    ...
    let tensors =
        SafeTensors::deserialize(&data).map_err(|e| LoadError::SafeTensors(e.to_string()))?;

    // 3. Build model structure
    let mut model = MambaLMHeadModel::new(config.clone());
    let _vocab_size = config.padded_vocab_size();
    let _d = config.d_model;
