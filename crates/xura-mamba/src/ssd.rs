...
pub fn mamba_chunk_scan_combined(
    ...
    if let Some(bias) = dt_bias {
        assert_eq!(bias.len(), nheads, "dt_bias shape mismatch");
    }
        ...
        for l in 0..seq_len {
            for h in 0..nheads {
                ...
                for p in 0..headdim {
                    ...
                    let out_idx =
                        b_idx * seq_len * nheads * headdim + l * nheads * headdim + h * headdim + p;
                    output[out_idx] = y;
                }
            }
        }
...
pub fn mamba2_ssm_step(
    ...
    if let Some(z_data) = z {
        assert_eq!(z_data.len(), bhd, "z shape mismatch");
    }
    if let Some(bias) = dt_bias {
        assert_eq!(bias.len(), nheads, "dt_bias shape mismatch");
    }
